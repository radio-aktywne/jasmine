...
export const authConfig = {
  ...
  cookies: {
    callbackUrl: {
      ...
      options: {
        domain: process.env.JASMINE__COOKIES__DOMAIN || undefined,
      },
    },
    csrfToken: {
  ...
  debug: (process.env.JASMINE__DEBUG || "true") === "true",
  pages: {
    error: "/auth/error",
    newUser: "/auth",
    signIn: "/auth/login",
  ...
  secret: splitSecrets(process.env.JASMINE__SECRETS__AUTH || "secret"),
  session: {
