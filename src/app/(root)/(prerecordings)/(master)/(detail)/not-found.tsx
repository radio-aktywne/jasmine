...
import { PrerecordingsDetailNotFoundView } from "./not-found.view";

async function getTitle({}: NotFoundMetadataUtilityInput = {}) {
  return msg({ message: "Not Found • jasmine" });
}
...
export default async function PrerecordingsDetailNotFound({}: NotFoundInput) {
  await connection();

  return (
    <>
      <Metadata title={await getTitle()} />
      <PrerecordingsDetailNotFoundView />
    </>
  );
}
